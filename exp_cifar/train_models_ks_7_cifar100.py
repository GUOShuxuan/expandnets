"""
Training script for SmallNet with kernel size 7*7 on cifar100
...
def train_cifar100_model(net, learning_rates=[0.001, 0.0001], iters=[50, 50],
    ...
    :param log_name: path to save log files
    :return:
    """
    ...
    for epoch in range(nepochs):
        if epoch < iters[0]:
            ...
            lr = learning_rates[2]
        print("Training with lr=%f for %d epoch" % (lr, epoch))
...
def train_cifar_models():
    ...
    train_cifar100_model(net, learning_rates=learning_rates, iters=iters,
                        output_path='results/models/tiny_cifar100_7_'+seed+'.model',
                        log_name='tiny_cifar100_7_'+seed)
    ...
    net.cuda()
    train_cifar100_model(net, learning_rates=learning_rates, iters=iters,
                        output_path='results/models/tiny_cifar100_7_enet_fc_' + seed + '.model',
                        log_name='tiny_cifar100_7_enet_fc_' + seed)
    ...
    net.cuda()
    train_cifar100_model(net, learning_rates=learning_rates, iters=iters,
                        output_path='results/models/tiny_cifar100_7_enet_cl_' + seed + '.model',
                        log_name='tiny_cifar100_7_enet_cl_' + seed)
    ...
    net.cuda()
    train_cifar100_model(net, learning_rates=learning_rates, iters=iters,
                        output_path='results/models/tiny_cifar100_7_enet_cl_fc_' + seed + '.model',
                        log_name='tiny_cifar100_7_enet_cl_fc_' + seed)

    # ExpandNet-CK
    print('***  ExpandNet-CK ***')
    net = Cifar_Tiny_ExpandNet_ck(num_classes=100)
    net.cuda()
    train_cifar100_model(net, learning_rates=learning_rates, iters=iters,
                        output_path='results/models/tiny_cifar100_7_enet_ck_' + seed + '.model',
                        log_name='tiny_cifar100_7_enet_ck_' + seed)

    # ExpandNet-CK+FC
    print('*** ExpandNet-CK+FC ***')
    net = Cifar_Tiny_ExpandNet_ck_fc(num_classes=100)
    net.cuda()
    train_cifar100_model(net, learning_rates=learning_rates, iters=iters,
                        output_path='results/models/tiny_cifar100_7_enet_ck_fc_' + seed + '.model',
                        log_name='tiny_cifar100_7_enet_ck_fc_' + seed)
...
def evaluate_cifar_models():
    ...
    evaluate_model(net=Cifar_Tiny(num_classes=100),
                   path='results/models/tiny_cifar100_7_' + seed + '.model',
                   result_path='results/evals/tiny_cifar100_7_'+seed+'.pickle',
                   dataset_name='cifar100', dataset_loader=cifar100_loader)

    print('***  ExpandNet-FC ***')
    evaluate_model(net=Cifar_Tiny_ExpandNet_fc(num_classes=100),
                   path='results/models/tiny_cifar100_7_enet_fc_' + seed + '.model',
                   result_path='results/evals/tiny_cifar100_7_enet_fc_' + seed + '.pickle',
                   dataset_name='cifar100', dataset_loader=cifar100_loader)

    print('***  ExpandNet-CL ***')
    evaluate_model(net=Cifar_Tiny_ExpandNet_cl(num_classes=100),
                   path='results/models/tiny_cifar100_7_enet_cl_' + seed + '.model',
                   result_path='results/evals/tiny_cifar100_7_enet_cl_' + seed + '.pickle',
                   dataset_name='cifar100', dataset_loader=cifar100_loader)

    print('***  ExpandNet-CL+FC ***')
    evaluate_model(net=Cifar_Tiny_ExpandNet_cl_fc(num_classes=100),
                   path='results/models/tiny_cifar100_7_enet_cl_fc_' + seed + '.model',
                   result_path='results/evals/tiny_cifar100_7_enet_cl_fc_' + seed + '.pickle',
                   dataset_name='cifar100', dataset_loader=cifar100_loader)

    print('***  ExpandNet-CK ***')
    evaluate_model(net=Cifar_Tiny_ExpandNet_ck(num_classes=100),
                   path='results/models/tiny_cifar100_7_enet_ck_' + seed + '.model',
                   result_path='results/evals/tiny_cifar100_7_enet_ck_' + seed + '.pickle',
                   dataset_name='cifar100', dataset_loader=cifar100_loader)

    print('***  ExpandNet-CK+FC ***')
    evaluate_model(net=Cifar_Tiny_ExpandNet_ck_fc(num_classes=100),
                   path='results/models/tiny_cifar100_7_enet_ck_fc_' + seed + '.model',
                   result_path='results/evals/tiny_cifar100_7_enet_ck_fc_' + seed + '.pickle',
                   dataset_name='cifar100', dataset_loader=cifar100_loader)


if __name__ == '__main__':

    print('Training networks with 7*7 kernels on CIFAR-100')
