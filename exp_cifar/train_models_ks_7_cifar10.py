"""
Training script for SmallNet with kernel size 7*7 on cifar10
...
parser = argparse.ArgumentParser(description='PyTorch Training Process CIFAR10')
...
def train_cifar10_model(net, learning_rates=[0.001, 0.0001], iters=[50, 50],
...
def train_cifar_models():
    ...
    net = Cifar_Tiny()
    net.cuda()
    train_cifar10_model(net, learning_rates=learning_rates, iters=iters,
                        output_path='results/models/tiny_cifar10_7_'+seed+'.model',
                        log_name='tiny_cifar10_7_'+seed)

    # FC(Arora18): ExpandNet-FC with er=4
    print('***  ExpandNet-FC ***')
    net = Cifar_Tiny_ExpandNet_fc()
    net.cuda()
    train_cifar10_model(net, learning_rates=learning_rates, iters=iters,
                        output_path='results/models/tiny_cifar10_7_enet_fc_' + seed + '.model',
                        log_name='tiny_cifar10_7_enet_fc_' + seed)

    # ExpandNet-CL
    print('***  ExpandNet-CL ***')
    net = Cifar_Tiny_ExpandNet_cl()
    net.cuda()
    train_cifar10_model(net, learning_rates=learning_rates, iters=iters,
                        output_path='results/models/tiny_cifar10_7_enet_cl_' + seed + '.model',
                        log_name='tiny_cifar10_7_enet_cl_' + seed)

    # ExpandNet-CL+FC
    print('*** ExpandNet-CL+FC ***')
    net = Cifar_Tiny_ExpandNet_cl_fc()
    net.cuda()
    train_cifar10_model(net, learning_rates=learning_rates, iters=iters,
                        output_path='results/models/tiny_cifar10_7_enet_cl_fc_' + seed + '.model',
                        log_name='tiny_cifar10_7_enet_cl_fc_' + seed)

    # ExpandNet-CK
    print('***  ExpandNet-CK ***')
    net = Cifar_Tiny_ExpandNet_ck()
    net.cuda()
    train_cifar10_model(net, learning_rates=learning_rates, iters=iters,
                        output_path='results/models/tiny_cifar10_7_enet_ck_' + seed + '.model',
                        log_name='tiny_cifar10_7_enet_ck_' + seed)

    # ExpandNet-CK+FC
    print('*** ExpandNet-CK+FC ***')
    net = Cifar_Tiny_ExpandNet_ck_fc()
    net.cuda()
    train_cifar10_model(net, learning_rates=learning_rates, iters=iters,
                        output_path='results/models/tiny_cifar10_7_enet_ck_fc_' + seed + '.model',
                        log_name='tiny_cifar10_7_enet_ck_fc_' + seed)
...
def evaluate_cifar_models():
    ...
    print('***  Baseline ***')
    evaluate_model(net=Cifar_Tiny(num_classes=10), path='results/models/tiny_cifar10_7_' + seed + '.model',
                             result_path='results/evals/tiny_cifar10_7_'+seed+'.pickle')

    print('***  ExpandNet-FC ***')
    evaluate_model(net=Cifar_Tiny_ExpandNet_fc(num_classes=10),
                             path='results/models/tiny_cifar10_7_enet_fc_' + seed + '.model',
                             result_path='results/evals/tiny_cifar10_7_enet_fc_' + seed + '.pickle')

    print('***  ExpandNet-CL ***')
    evaluate_model(net=Cifar_Tiny_ExpandNet_cl(num_classes=10),
                             path='results/models/tiny_cifar10_7_enet_cl_' + seed + '.model',
                             result_path='results/evals/tiny_cifar10_7_enet_cl_' + seed + '.pickle')

    print('***  ExpandNet-CL+FC ***')
    evaluate_model(net=Cifar_Tiny_ExpandNet_cl_fc(num_classes=10),
                             path='results/models/tiny_cifar10_7_enet_cl_fc_' + seed + '.model',
                             result_path='results/evals/tiny_cifar10_7_enet_cl_fc_' + seed + '.pickle')

    print('***  ExpandNet-CK ***')
    evaluate_model(net=Cifar_Tiny_ExpandNet_ck(num_classes=10),
                   path='results/models/tiny_cifar10_7_enet_ck_' + seed + '.model',
                   result_path='results/evals/tiny_cifar10_7_enet_ck_' + seed + '.pickle')

    print('***  ExpandNet-CK+FC ***')
    evaluate_model(net=Cifar_Tiny_ExpandNet_ck_fc(num_classes=10),
                   path='results/models/tiny_cifar10_7_enet_ck_fc_' + seed + '.model',
                   result_path='results/evals/tiny_cifar10_7_enet_ck_fc_' + seed + '.pickle')


if __name__ == '__main__':

    print('Training networks with 7*7 kernels on CIFAR-10')
